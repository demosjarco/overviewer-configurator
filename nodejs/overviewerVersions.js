...
function updateOverviewerVersions(latestVersionCallback = null, versionsCallback = null, errorCallback = null) {
	const os = require('os');
	let osType = '';
	switch (os.platform()) {
		case 'darwin':
			...
			if (builds.length > 0) {
				...
				function buildLoop(buildNumber) {
					request('https://overviewer.org/build/api/v2/builders/' + osType + '/builds/' + buildNumber + '/steps/upload', function (error2, response2, body2) {
						...
						buildLoopCounter++;
						goodBuildLimit++;
						if (buildLoopCounter < builds.length && goodBuildLimit < 25) {
