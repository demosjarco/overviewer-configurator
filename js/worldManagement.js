...
ipcRenderer.on('clearWorlds', function (event, arg) {
	$('body aside ul li.worldItem').remove();
	$('body main div.worldItem').remove();
	redoTabs();
	tabCounter = 2;
});

ipcRenderer.on('gotWorld', function (event, worldNickName, worldName, worldPath) {
	$('body aside ul').append('<li class="worldItem"><a class="worlds" href="#tabs-' + tabCounter + '">[' + worldNickName + ']</a></li>');
...
ipcRenderer.on('gotWorldSettings', function (event, worldNickName, json) {
	Object.keys(json.directions).forEach(function (direction) {
		if (json.directions[direction]) {
			$('body main div.worldItem.' + worldNickName + ' div.directions div.direction.' + direction).addClass('selected');
		} else {
			$('body main div.worldItem.' + worldNickName + ' div.directions div.direction.' + direction).removeClass('selected');
		}
	});

	Object.keys(json.renderTypes).forEach(function (renderType) {
		$('body main div.worldItem.' + worldNickName + ' input:checkbox.enable.' + renderType).prop('checked', json.renderTypes[renderType].enabled);
		if (('smoothLighting' in json.renderTypes[renderType]))
			$('body main div.worldItem.' + worldNickName + ' input:checkbox.smoothLighting.' + renderType).prop('checked', json.renderTypes[renderType].smoothLighting);
		$('body main div.worldItem.' + worldNickName + ' details select.renderType.' + renderType).val(json.renderTypes[renderType].updateMode);
	});
});