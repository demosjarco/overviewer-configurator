...
ipcRenderer.on('newOverviewerVersions', function (event, version, url) {
	let newVersion = $('<li class="allowed v' + version.replace(/\./g, '_') + '"><div class="content"><span class="status"><i class="material-icons">' + (installedVersion == version ? 'cloud_done' : 'cloud_download') + '</i></span><span>' + version + '</span></div></li>');
